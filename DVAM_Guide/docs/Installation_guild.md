...
!!! note
To avoid potential dependency conflicts, installing within a `conda` environment is recommended. And using `pip install -U metaiverse` to update.
...
The `metaverse` package can be installed via pip using one of the following commands:
...
2. After the installation of pytorch, we can start to install `metaverse` by `pip`

   ```shell
   pip install -U metaverse
   pip install -U numba
   ```
